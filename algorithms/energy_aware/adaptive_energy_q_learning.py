...
class AdaptiveEnergyQLearningAgent:
    ...
    def train(self):
        ...
        for episode in range(EPISODES):
            ...
            for _ in range(MAX_STEPS):
                ...
                # Calculate adaptive energy-aware reward
                reward, energy, lambda_t = calculate_adaptive_reward(
                    env.state
                )
            ...
            if lambda_values:
                episode_lambdas.append(
                    sum(lambda_values) / len(lambda_values)
                )
            else:
                episode_lambdas.append(0.0)

        return {
            "rewards": episode_rewards,
            "energies": episode_energies,
            "lambdas": episode_lambdas
        }